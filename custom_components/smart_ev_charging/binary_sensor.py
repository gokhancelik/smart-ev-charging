...
def _is_unavailable(raw: str | None) -> bool:
    ...
    Some integrations emit decorated variants of the real value, e.g. the
    Easee status sensor's transient ``"unknown 0"``. Rather than failing
    to match ``"unknown 0" in on_states`` (which would surface as a fake
    unplug), treat any state whose first word is an unavailable marker as
    unavailable itself.
    """
...
async def async_setup_entry(
    ...
    # Order matters: the charging mirror must be added (and thus readable
    # via hass.states) BEFORE the connected mirror, because the connected
    # mirror consults it via `implied_on_by` at async_added_to_hass time.
    async_add_entities(
        [
            ...
            SmartEvChargingMirrorBinarySensor(
                ...
                # A vehicle that is drawing current is necessarily plugged
                # in, regardless of what the connected source reports — so
                # even a misconfigured connected states list (missing
                # "charging") can't drop the connected mirror mid-session.
...
class SmartEvChargingMirrorBinarySensor(BinarySensorEntity):
    ...
    def __init__(
        ...
        *,
        ...
        implied_on_by: str | None = None,
    ) -> None:
        self._source_entity_id = source_entity_id
        self._implied_on_by = implied_on_by
    ...
    async def async_added_to_hass(self) -> None:
        ...
        # Watch the implying entity too, so the connected mirror re-evaluates
        # when charging starts — that transition must flip connected back on.
        watch = [self._source_entity_id]
        if self._implied_on_by:
            watch.append(self._implied_on_by)
        self.async_on_remove(
            async_track_state_change_event(
                self.hass, watch, self._handle_source_event
            )
        )
    ...
    def _apply_source_state(self, state) -> None:
        ...
        if self._implied_on_by and self.hass.states.is_state(self._implied_on_by, "on"):
